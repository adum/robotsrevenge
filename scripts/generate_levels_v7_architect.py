...
BIOMES = [
    "checkerboard", "checkerboard_inv", "h_stripes", "v_stripes", 
    "h_stripes_2", "v_stripes_2", "diagonal", "dense", "sparse", "empty"
]


def generate_bsp_rooms(width, height, target_rooms, rng):
    rooms = [(0, 0, width, height)]
    ...
    while len(rooms) < target_rooms:
        ...
        split_dir = "h" if can_split_h and (not can_split_v or rng.random() < 0.5) else "v"
        
        if split_dir == "h":
            split_y = rng.randint(4, h - 4)
            rooms.append((x, y, w, split_y))
            rooms.append((x, y + split_y, w, h - split_y))
        else:
            split_x = rng.randint(4, w - 4)
            rooms.append((x, y, split_x, h))
            rooms.append((x + split_x, y, w - split_x, h))
            
    return rooms


def get_biome_value(biome_type, x, y, rng):
    if biome_type == "checkerboard":
        return (x + y) % 2 == 0
    elif biome_type == "checkerboard_inv":
        return (x + y) % 2 == 1
    elif biome_type == "h_stripes":
        return y % 2 == 0
    elif biome_type == "v_stripes":
        return x % 2 == 0
    elif biome_type == "h_stripes_2":
        return y % 3 == 0
    elif biome_type == "v_stripes_2":
        return x % 3 == 0
    elif biome_type == "diagonal":
        return (x - y) % 3 == 0
    elif biome_type == "dense":
        return rng.random() < 0.65
    elif biome_type == "sparse":
        return rng.random() < 0.15
    elif biome_type == "empty":
        return False
    return False


def build_base_board(width, height, rooms, rng):
    board = [[False for _ in range(width)] for _ in range(height)]
    room_biomes = {}
    for r in rooms:
        room_biomes[r] = rng.choice(BIOMES)
        ...
        for x in range(width):
            ...
            for r in rooms:
                ...
                if rx <= x < rx + rw and ry <= y < ry + rh:
                    my_biome = room_biomes[r]
                    break
            board[y][x] = get_biome_value(my_biome, x, y, rng)
            
    return board


def _build_architect_trace(program, start_x, start_y, width, height, max_steps, max_carves, min_steps, base_board, rng):
    n = len(program)
    best_result = None
    nodes_explored = 0

    def dfs(x, y, dir_index, pc, steps, reqs, state_visits, jump_exec, sense_true, sense_false, decoy_spawns, carves):
        nonlocal best_result, nodes_explored
        nodes_explored += 1
        if nodes_explored > 35000:
            return False
        ...
        if op == "F":
            ...
            if forced is True:
                ...
                if base_board[ny][nx] == True:
                    new_carves += 1
                    if new_carves > max_carves:
                        return False
            
            new_reqs = reqs.copy()
            new_reqs[(nx, ny)] = False
            return dfs(nx, ny, dir_index, core.wrap(pc + 1, n), steps + 1, new_reqs, new_visits, jump_exec, sense_true, sense_false, decoy_spawns, new_carves)

        if op == "L":
            return dfs(x, y, core.wrap(dir_index - 1, 4), core.wrap(pc + 1, n), steps + 1, reqs, new_visits, jump_exec, sense_true, sense_false, decoy_spawns, carves)
        ...
        if op == "J":
            ...
            return dfs(x, y, dir_index, core.wrap(pc + offset, n), steps + 1, reqs, new_visits, jump_exec + 1, sense_true, sense_false, decoy_spawns, carves)
        ...
        if op == "S":
            ...
            if not core.in_bounds(nx, ny, width, height):
                return dfs(x, y, dir_index, core.wrap(pc + 2, n), steps + 1, reqs, new_visits, jump_exec, sense_true, sense_false + 1, decoy_spawns, carves)
            ...
            if forced is not None:
                ...
                base_val = base_board[ny][nx]
                if rng.random() < 0.8:
                    choices = [(base_val, 0), (not base_val, 1)]
                else:
                    choices = [(not base_val, 1), (base_val, 0)]

            success = False
            for blocked, cost in choices:
                if carves + cost > max_carves:
                    continue
                ...
                if blocked:
                    ...
                    if dfs(x, y, dir_index, core.wrap(pc + 1, n), steps + 1, new_reqs, new_visits, jump_exec, sense_true + 1, sense_false, new_decoys, carves + cost):
                        success = True
                        if cost == 0: return True 
                else:
                    new_decoys.append((x, y, dir_index, core.wrap(pc + 1, n)))
                    if dfs(x, y, dir_index, core.wrap(pc + 2, n), steps + 1, new_reqs, new_visits, jump_exec, sense_true, sense_false + 1, new_decoys, carves + cost):
                        success = True
                        if cost == 0: return True
            return success
    ...
    dfs(start_x, start_y, core.NORTH_DIR, 0, 0, initial_reqs, {}, 0, 0, 0, [], 0)
...
def _run_decoy_trace(program, start_x, start_y, start_dir, start_pc, reqs, base_board, width, height, max_steps, rng):
    x, y = start_x, start_y
    dir_index = start_dir
    pc = start_pc
    steps = 0
    ...
    while steps < max_steps:
        ...
        if op == "S":
            ...
            if forced is not None:
                ...
                base_val = base_board[ny][nx]
                if base_val == False and rng.random() < 0.25:
                    blocked = True
                    reqs[(nx, ny)] = True
                else:
                    blocked = base_val
...
def _architect_random_program(length, rng):
    return core._random_program(length, rng)


def _try_generate_architect_level(level_id, options, rng):
    start_x = options.width // 2
    start_y = options.height // 2

    target_rooms = max(2, int(math.sqrt(options.width * options.height) / 4))
    rooms = generate_bsp_rooms(options.width, options.height, target_rooms, rng)
    base_board = build_base_board(options.width, options.height, rooms, rng)
    base_board[start_y][start_x] = False

    hidden_solution = _architect_random_program(options.solution_length, rng)
    if core.has_meaningless_jump_instruction(hidden_solution):
        return None, "mj"

    max_carves = max(20, options.width)

    min_interesting_steps = max(
        10, math.floor((options.width + options.height) * options.min_steps_size_factor)
    )

    trace_result = _build_architect_trace(
        hidden_solution, start_x, start_y, options.width, options.height, options.execution_limit, max_carves, min_interesting_steps, base_board, rng
    )
    ...
    for dx, dy, ddir, dpc in decoys:
        success = _run_decoy_trace(
            hidden_solution, dx, dy, ddir, dpc, reqs, base_board, options.width, options.height, 40, rng
        )
        if not success:
            return None, "de"
    ...
    # Don't strictly reject density because Biomes control density naturally
    # However, just doing a sanity check
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "V7 Architect generator. Splits the board into rooms with distinct geometric 'biomes', "
            "then searches for a program that can naturally navigate or minimally carve its way out."
        )
    )
...
def main(argv: list[str]) -> int:
    ...
    print(
        f"Generating V7 levels {args.start_level}..{args.max_level} "
    ...
    for level_number in range(args.start_level, args.max_level + 1):
        ...
        while len(candidate_pool) < args.best_of and (options.max_attempts == 0 or evals < options.max_attempts):
            ...
            generated, reject_code = _try_generate_architect_level(level_number, options, rng)
