...
def _build_v5_trace(program, start_x, start_y, width, height, max_steps, rng):
    n = len(program)
    best_result = None
    nodes_explored = 0
    ...
    def dfs(x, y, dir_index, pc, steps, reqs, state_visits, jump_exec, sense_true, sense_false, decoy_spawns):
        ...
        if visit_count > 2:  # prevent infinite loops during search
            return False
            ...
            for blocked in choices:
                ...
                if blocked:
                    ...
                    if dfs(x, y, dir_index, core.wrap(pc + 1, n), steps + 1, new_reqs, new_visits, jump_exec, sense_true + 1, sense_false, new_decoys):
                        success = True
                        if len(choices) > 1: return True # Just return first valid path to save time
                else:
...
def _apply_ca_smoothing(board, reqs, width, height, steps, rng):
    # Swap-based or milder CA to preserve density
...
def _v5_random_program(length, rng):
    if length < 5:
        return core._random_program(length, rng)

    program = []
    
    # Idiom: S (jump to turn) F F F ... J (back to S) L/R (wrap to S)
    # The F section must have at least 1 F to make progress.
    # Total length: 1(S) + 1(J) + f_count + 1(J_back) + turn_count
    # So length = 3 + f_count + turn_count
    # We will put all extra length into the F section (f_count) or turn section (turn_count).
    
    f_count = max(1, length - 4)
    turn_count = length - 3 - f_count
...
def _try_generate_v5_level(level_id, options, rng):
    start_x = options.width // 2
    start_y = options.height // 2

    hidden_solution = _v5_random_program(options.solution_length, rng)
    if core.has_meaningless_jump_instruction(hidden_solution):
        return None, "mj"

    trace_result = _build_v5_trace(
        hidden_solution, start_x, start_y, options.width, options.height, options.execution_limit, rng
    )
    ...
    # board = _apply_ca_smoothing(board, reqs, options.width, options.height, 3, rng)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "V5 Vision-aligned level generator. Prioritizes meaningful logic, "
    ...
    parser.add_argument("--max-program-length", type=int, default=14, help="Maximum hidden-solution length.")
    ...
    parser.add_argument("--max-attempts", type=int, default=2500, help="Max generation attempts per level.")
...
def main(argv: list[str]) -> int:
    ...
    print(
        f"Generating V5 levels {args.start_level}..{args.max_level} "
    ...
    for level_number in (args.level_number,):
        ...
        options = core.GenerateOptions(
            ...
            execution_limit=800 + current_size * 10,
        ...
        while len(candidate_pool) < args.best_of and evals < options.max_attempts:
            ...
            generated, reject_code = _try_generate_v5_level(level_number, options, rng)
        ...
        # Select best candidate (maximize steps)
